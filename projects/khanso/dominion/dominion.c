...
/* card effects for use with cardEffect */
  ...
  while(drawntreasure<=2){
      if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
          ...
          if (state->deckCount[currentPlayer] < 1) {
            // player has zero cards and should return -1
            return -1;
          }
      }
      drawCard(currentPlayer, state);
...
int stewardEffect(int currentPlayer, int handPos, struct gameState *state, int choice1, int choice2, int choice3)
{
    if (choice1 == 2)
    {
        //+2 cards
...
int remodelEffect(int currentPlayer, int handPos, struct gameState *state, int choice1, int choice2)
{
    int i, j;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case remodel:
      return remodelEffect(currentPlayer, handPos, state, choice1, choice2);
    ...
    case steward: 
      return stewardEffect(currentPlayer, handPos, state, choice1, choice2, choice3);
